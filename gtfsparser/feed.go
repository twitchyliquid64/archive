...
type Feed struct {
	...
	Options	       map[OptType]*Opt

	inMemoryZip   *zip.Reader
...
func NewFeed() *Feed {
	g := Feed{
		...
		Options:	make(map[OptType]*Opt),
	}
	return &g
}

//set an option for the parser
func (feed *Feed) EnableOption(optT OptType){
	feed.Options[optT] = &Opt{enabled: true}
}

// Parse a GTFS formatted in-memory ZIP structure (from archive/zip) into the feed
func (feed *Feed) ParseZip(zfile *zip.Reader) error{
	feed.inMemoryZip = zfile
	return feed.Parse("")
}
...
func (feed *Feed) getFile(path string, name string) (io.Reader, error) {
	if feed.inMemoryZip != nil { //not using path
		for _, f := range feed.inMemoryZip.File {
			if f.Name == name {
				return f.Open()
			}
		}
...
func (feed *Feed) parseTrips(path string) (err error) {
	...
	for record = reader.ParseRecord(); record != nil; record = reader.ParseRecord() {
		var trip *gtfs.Trip
		trip = createTrip(record, feed.Routes, feed.Services, feed.Shapes, feed)
