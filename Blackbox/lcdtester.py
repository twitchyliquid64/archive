...
class i2c_device:
 def __init__(self, addr, port, andbyte=0, andenable=False):
  self.addr = addr
  self.bus = smbus.SMBus(port)
  self.andbyte = andbyte
  self.andenable = andenable

 def write(self, byte):
  if self.andenable:
    self.bus.write_byte(self.addr, byte | self.andbyte)
  else:
...
class lcd:
 ...
 '''
 def __init__(self, addr, port, reverse=0, backlightPort=0):
  self.reverse = reverse
  self.lcd_device = i2c_device(addr, port, backlightPort)
  if backlightPort > 0:
    self.lcd_device.andenable = True
  if self.reverse:
 ...
 def backlight(self, status):
  self.lcd_device.andenable = status
  self.lcd_puts("", 3)

display = lcd(0x27, 1, 1, 8)
display.lcd_clear()
display.lcd_puts("ALERT!!!", 1)
status = False
while True:
